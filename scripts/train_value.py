...
def init_train_state(
    ...
    # Compute sharding for partial params to avoid replication across devices during initialization
    params_shape = train_state_shape.params.to_pure_dict()
    params_sharding = sharding.fsdp_sharding(params_shape, mesh, log=False)

    # Extract sharding specs that match the partial_params structure
    # partial_params may only contain a subset of params_shape (e.g., base weights without LoRA)
    flat_partial = traverse_util.flatten_dict(partial_params, sep="/")
    flat_sharding = traverse_util.flatten_dict(params_sharding, sep="/")

    # Convert numpy arrays to sharded JAX arrays before jit compilation
    # This avoids copying weights to all GPUs during initialization
    ...
    for key, arr in flat_partial.items():
        if key in flat_sharding and isinstance(arr, np.ndarray):
            ...
            if isinstance(shard_spec, jax.sharding.NamedSharding):
                # Convert numpy array to JAX array with the specified sharding
    ...
    # Build sharding tree that matches partial_params structure for in_shardings
    matching_sharding = {
    ...
    # Use sharded input sharding to avoid copying weights to all GPUs
    train_state = jax.jit(
        init,
        donate_argnums=(1,),  # donate the partial params buffer.
        in_shardings=(None, partial_params_sharding),  # rng is None (replicated), params are sharded
...
def train_step(
    ...
    """Training step for value model.

    Args:
        config: Training configuration
        rng: Random key
        state: Current training state
        batch: Tuple of (observation, actions, value_targets)
               Note: actions are included for compatibility but not used by value model
    ...
    def loss_fn(
        ...
        For value models, we pass value_targets instead of actions.
        """
        # Value model's compute_loss expects value_targets parameter
