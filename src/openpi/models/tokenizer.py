"""
model weights and tokenizer for value model : 
    1. Kaggle:  https://www.kaggle.com/models/google/gemma-3-270m
        # NOTE: you could also use the code block supported by kaggle: 
            import kagglehub

            # Download latest version
            path = kagglehub.model_download("google/gemma-3/flax/gemma-3-270m")
...
class ValueGemmaTokenizer:
    """Tokenizer for ValueGemma models that loads tokenizer from model_path."""

    def __init__(self, max_len: int, model_path: str):
        self._max_len = max_len
        import gemma.gm as gm
        self._tokenizer = gm.text.Gemma3Tokenizer(os.path.join(model_path, "tokenizer.model"))

    def tokenize(self, prompt: str, state: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
        """Tokenize a prompt and optionally state into tokens and mask.

        Args:
            prompt: The text prompt to tokenize.
            state: Optional state array to discretize and include in the prompt.

        Returns:
            A tuple of (tokens, token_mask) arrays.
        """
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        if state is not None:
            # This is the Pi05 format, where the state is part of the discrete language input.
...
class PaliGemmaStarTokenizer:
    def __init__(self, 
                 fast_max_len: int = 256, 
                 star_max_len: int = 200,
                 fast_tokenizer_path: str = "physical-intelligence/fast", 
                 
                 ):
        self._fast_max_len = fast_max_len
        self._star_max_len = star_max_len  
    ...
    def fast_tokenize(
        ...
        # Pad tokens to max length
        tokens_len = len(tokens)
        if tokens_len < self._fast_max_len:
            padding = [False] * (self._fast_max_len - tokens_len)
            tokens = tokens + padding
            token_mask = token_mask + padding
            ar_mask = ar_mask + padding
            loss_mask = loss_mask + padding
        else:
            if len(tokens) > self._fast_max_len:
                logging.warning(
                    f"Token length ({len(tokens)}) exceeds fast max length ({self._fast_max_len}), truncating. "
                    "Consider increasing the `max_token_len` in your model config if this happens frequently."
                )
            tokens = tokens[: self._fast_max_len]
            token_mask = token_mask[: self._fast_max_len]
            ar_mask = ar_mask[: self._fast_max_len]
            loss_mask = loss_mask[: self._fast_max_len]

        return np.asarray(tokens), np.asarray(token_mask), np.asarray(ar_mask), np.asarray(loss_mask)
    
    def star_tokenize(self, prompt: str, state: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        if state is not None:
            # This is the Pi05 format, where the state is part of the discrete language input.
            discretized_state = np.digitize(state, bins=np.linspace(-1, 1, 256 + 1)[:-1]) - 1
            state_str = " ".join(map(str, discretized_state))
            full_prompt = f"Task: {cleaned_text}, State: {state_str};\nAction: "
            tokens = self._paligemma_tokenizer.encode(full_prompt, add_bos=True)
        else:
            # This is the Pi0 format, where the state is part of the continuous action expert input.
            # tokenize "\n" separately as the "start of answer" token
            tokens = self._paligemma_tokenizer.encode(cleaned_text, add_bos=True) + self._paligemma_tokenizer.encode("\n")
        tokens_len = len(tokens)
        if tokens_len < self._star_max_len:
            padding = [False] * (self._star_max_len - tokens_len)
            mask = [True] * tokens_len + padding
            tokens = tokens + padding
        else:
            if len(tokens) > self._star_max_len:
                logging.warning(
                    f"Token length ({len(tokens)}) exceeds star max length ({self._star_max_len}), truncating. "
                    "Consider increasing the `max_token_len` in your model config if this happens frequently."
                )
            tokens = tokens[: self._star_max_len]
            mask = [True] * self._star_max_len
