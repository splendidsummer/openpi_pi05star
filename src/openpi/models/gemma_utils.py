...
def build_gemma_3_270m_model(model_path=None, rng_key=None):
    ...
    """
    # 验证检查点路径存在（如果提供了本地路径）
    # 如果 model_path 为 None，则不进行加载
    if model_path is not None and not model_path.startswith('gs://') and model_path.lower() != "online":
        checkpoint_path = pathlib.Path(model_path)
        if not checkpoint_path.exists():
            raise FileNotFoundError(
                f"检查点路径不存在: {checkpoint_path}\n"
                "请确保路径正确，或使用 model_path=None 使用在线检查点"
            )
    ...
    if model_path is not None:
        ...
        # 确定使用在线还是本地
        use_online = (model_path.lower() == "online" or model_path.startswith("gs://"))
        # 如果是 "online"，传给 test_load_checkpoint 的路径应该是 None (让其使用默认) 或 gs URI
        path_arg = model_path if model_path.lower() != "online" else None
        
        # 使用现有的 test_load_checkpoint 函数加载参数
        # 这个函数会进行完整性检查、参数数量验证等
        loaded_params = test_load_checkpoint(model, checkpoint_path=path_arg, use_online=use_online)
        
        if loaded_params is None:
            raise RuntimeError(f"无法从 {model_path} 加载模型参数")
            
        # 验证参数数量是否匹配
        # 1. 初始化 Flax Linen 模型以获取预期的参数结构
        # Flax 需要使用 dummy 数据运行一次来确定参数形状，这里是为了计算模型定义的总参数量
        # 这里的序列长度 (8) 是任意的，只要是合法的形状 (batch, seq_len) 即可，不会影响模型参数的形状
        ...
        n_init = count_params(init_params)
        n_loaded = count_params(loaded_params)
        
        if n_init != n_loaded:
            raise ValueError(f"参数数量不匹配: 模型定义需要 {n_init} 个参数，但从检查点加载了 {n_loaded} 个")
