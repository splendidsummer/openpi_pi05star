    ...
    def __call__(self, model_config: _model.BaseModelConfig) -> _transforms.Group:
        match model_config.model_type:
            ...
            case _model.ModelType.VALUE:
                assert isinstance(model_config, value_config.ValueConfig)
                return _transforms.Group(
                    inputs=[
                        _transforms.InjectDefaultPrompt(self.default_prompt),
                        _transforms.ResizeImages(224, 224),
                        #  TODO: in the future we have to include FastTokenizer as well
                        _transforms.TokenizePrompt(
                            # _tokenizer.FastTokenzier ? 
                            _tokenizer.ValueGemmaTokenizer(model_config.max_token_len, model_config.model_path),
                            discrete_state_input=model_config.discrete_state_input,
                        ),
                        _transforms.PadStatesAndActions(model_config.action_dim),
                    ],
...
class DataConfigFactory(abc.ABC):
    ...
    def create_base_config(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        assets_dir_override = epath.Path(self.assets.assets_dir) if self.assets.assets_dir else None
        norm_stats = self._load_norm_stats(
            assets_dirs=epath.Path(assets_dirs),
    ...
    def _load_norm_stats(
        self,
        *,
        assets_dirs: epath.Path,
        assets_dir_override: epath.Path | None,
        asset_id: str | None,
        repo_id: str | None,
        ...
        if assets_dir_override is not None:
            if asset_id is not None:
                candidates.append(assets_dir_override / asset_id)
            else:
                candidates.append(assets_dir_override)
                if repo_id is not None:
                    candidates.append(assets_dir_override / repo_id)
        else:
            if asset_id is not None:
                candidates.append(assets_dirs / asset_id)
            elif repo_id is not None:
                candidates.append(assets_dirs / repo_id)
            else:
                return None

        for candidate in candidates:
            try:
                data_assets_dir = str(candidate)
                norm_stats = _normalize.load(_download.maybe_download(data_assets_dir))
                logging.info(f"Loaded norm stats from {data_assets_dir}")
                return norm_stats
            except FileNotFoundError:
                logging.info(f"Norm stats not found in {candidate}, skipping.")
...
@dataclasses.dataclass(frozen=True)
class LeRobotDROIDValueDataConfig(DataConfigFactory):
    """
    Example data config for custom DROID dataset in LeRobot format.
    To convert your custom DROID dataset (<10s of hours) to LeRobot format, see examples/droid/convert_droid_data_to_lerobot.py
    """
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_droid_value",
        model=value_config.ValueInferenceConfig(
        ...
        data=SimpleDataConfig(
            ...
            assets=AssetsConfig(
                assets_dir="/root/autodl-tmp/openpi_pi05star/assets/pi05_droid_100_value",
                asset_id="SummerZhang/droid_100",
            ),                                                          
            data_transforms=lambda model: _transforms.Group(
                inputs=[droid_policy.DroidInputs(model_type=ModelType.VALUE)],
                # outputs=[droid_policy.DroidOutputs()],
            ),
            base_config=DataConfig(
                prompt_from_task=True,
    ...
    TrainConfig(
        ...
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        ema_decay=None,  # Disabled to reduce memory usage
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        pytorch_weight_path="/path/to/your/pytorch_weight_path",
        num_train_steps=30_000,
    ),
    ...
    TrainConfig(
        ...
        log_interval=100,
        save_interval=5000,
        keep_period=10_000,  #  TODO: adjust based on num_train_steps? 'gn6hs de2jjjjjjj
    ...
    TrainConfig(
        ...
        model=value_config.ValueConfig(
            pi05=True,
            model_path="/root/autodl-tmp/gemma-3-270m",
            # Value range for remaining timesteps: V(t) = -1 * (episode_length - t) + 1
            Vmin=-1.0,  # For max episode length ~200: -(200-1) = -199
            Vmax=1.0,     # Maximum value at episode end
            # action_dim=32,  # pi05 is trained with 32-dim actions
            # action_horizon=16,
        ),
        ...
        lr_schedule=_optimizer.MultiLRScheduleConfig(
            param_lr_schedules={
                ...
                "value_head": _optimizer.CosineDecaySchedule(
                    warmup_steps=1_000,
                    peak_lr=1e-4,  # Value head 学习率（比 SigLIP 高 10 倍）
                    decay_steps=1_000_000,
                    decay_lr=1e-4,
                ),
            },
