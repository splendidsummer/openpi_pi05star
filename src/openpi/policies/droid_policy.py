...
def load_parquet_episode():
    """Loads a sample episode from a parquet file for testing."""
    import pyarrow.parquet as pq

    path = (
        "gs://le-robot-data/openpi/droid/droid_pi0_episode_parquets/"
        "droid_pi0_episode_00000000.parquet"
    )
...
def make_droid_value_example() -> dict:
    """Creates a random input example for the Droid policy."""
    return {
        "observation/exterior_image_1_left": np.random.randint(256, size=(224, 224, 3), dtype=np.uint8),
        ...
        "observation/joint_position": np.random.rand(7),
        "observation/gripper_position": np.random.rand(1),
        "prompt": "move the plate on top of the tower",
    }
...
class DroidInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        match self.model_type:
            ...
            #     # For temp use to run the inference 
            #     names = ("base_0_rgb", "base_1_rgb", "wrist_0_rgb")
...
@dataclasses.dataclass(frozen=True)
class DroidValueInferInputs(transforms.DataTransformFn):
    # Determines which model will be used.
    model_type: _model.ModelType
    ...
    def __call__(self, data: dict) -> dict:
        ...
        match self.model_type:
            case _model.ModelType.VALUE:
                names = ("base_0_rgb", "base_1_rgb", "wrist_0_rgb")
                images = (base_image_1, base_image_2, wrist_image)
                image_masks = (np.True_, np.True_, np.True_)
            case _model.ModelType.PI05_STAR:
                names = ("base_0_rgb", "base_1_rgb", "wrist_0_rgb")
                images = (base_image_1, base_image_1,  wrist_image)
...
if __name__ == "__main__":
    _model_type = _model.ModelType.PI05_STAR
    _droid_inputs = DroidInputs(model_type=_model_type)

    _example = make_droid_example()
    # Add fake adv_indicator for PI05_STAR
    _example["adv_indicator"] = np.array([True])
    
    print("Example keys:", _example.keys())
    
    _outs = _droid_inputs(_example)
    print("Output keys:", _outs.keys())
    
    if "image" in _outs:
        print("Image keys:", _outs["image"].keys())
        for k, v in _outs["image"].items():
